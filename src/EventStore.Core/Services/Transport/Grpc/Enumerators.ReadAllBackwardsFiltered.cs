		...
		public class ReadAllBackwardsFiltered : IAsyncEnumerator<ResolvedEvent> {
			...
			public async ValueTask<bool> MoveNextAsync() {
				...
				_bus.Publish(new ClientMessage.FilteredReadAllEventsBackward(
					...
					_resolveLinks, false, _maxSearchWindow, default, _eventFilter, _user, expiresAt: _expiresAt));
				...
				void OnMessage(Message message) {
					...
					if (!(message is ClientMessage.FilteredReadAllEventsBackwardCompleted completed)) {
						readNextSource.TrySetException(
							RpcExceptions
								.UnknownMessage<ClientMessage.FilteredReadAllEventsBackwardCompleted>(message));
						return;
					}
